...
// this is not dynamic
const labels = ["参与人数", "未参与人数"];

function MultipleStudentsTotalParticipationChart({
  height,
  dataSet,
}: {
  height?: number;
      ...
      plugins: [
        {
          ...
          afterDraw: function (chart) {
            if (selectedDatasetIndex >= 0) {
              ...
              ctx.fillText("参与率", centerX, centerY);
              ctx.restore();
            }
          },
        },
        ...
        <div className="flex flex-wrap gap-y-2 gap-6">
          {dataSet.map((item, index) => (
            <div key={item.label} className="flex flex-col items-left">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400 white">
                <span className="pr-2">总人数：{item.data[0] + item.data[1]}</span>
                <span>
                  参与人数：{item.data[0]} (
...
export default MultipleStudentsTotalParticipationChart;
