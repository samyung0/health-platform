...
const dataFetched = [
  {
    label: "全部",
    data: [34, 67, 69, 73, 75, 77, 79, 81, 83],
  },
];
const exerciseTypes = [
  "50米跑",
...
function MultipleStudentsTotalGradeChart({ height }: { height?: number }) {
  ...
  const [colors, hoverColors] = useMemo(
    () => getChartColor(exerciseTypes.length),
    ...
    () => ({
      ...
      datasets: dataFetched
        ...
        .map((item, index) => ({
          label: item.label,
          data: item.data,
          backgroundColor: colors.map((color) => adjustColorOpacity(color, 0.85)),
          hoverBackgroundColor: hoverColors.map((color) => adjustColorOpacity(color, 1)),
          borderWidth: 4,
          borderRadius: 4,
          cutout: "80%",
        })),
        ...
        tooltip: {
          callbacks: {
            label: (context: TooltipItem<"doughnut">) => {
              return `${context.dataset.label}: ${context.parsed}分钟 (${Math.round(
      ...
      plugins: [
        {
          ...
          afterDraw: function (chart) {
            if (selectedDatasetIndex >= 0) {
              ...
              ctx.fillText(
                (chart.config.data.labels as string[])[selectedIndex] ?? "",
                centerX,
                centerY
              );
              ctx.restore();
      ...
      <div className="px-5 pt-3 pb-6">
        <div className="flex flex-wrap gap-y-1 gap-3">
          {exerciseTypes.map((item, index) => (
            <div key={item} className="flex flex-col items-left">
