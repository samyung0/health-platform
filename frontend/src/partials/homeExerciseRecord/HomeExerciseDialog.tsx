...
import type { ExerciseName } from "@/db/schema";
...
export default function HomeExerciseDialog({
  open,
  setOpen,
}: {
  open: boolean;
  ...
  const [request, setRequest] = useState<{
    recordType: ExerciseName | undefined;
                    ...
                    <div className="flex flex-col flex-1">
                      <label className="block text-sm font-medium mb-1">
                        ...
                        {["50米跑", "50米×8往返跑"].includes(request.recordType)
                          ? "米"
                          : measureType.find(
                              (measure) => measure.exerciseName === request.recordType
                            )?.unit}
