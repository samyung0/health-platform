...
class NationalStandardConverter:
    ...
    def calculate_all_scores_by_standards(self, df):
        ...
        # 计算BMI
        df["BMI"] = df.apply(
            lambda row: self.calculate_bmi(row["身高(cm)"], row["体重(kg)"]), axis=1
        )
    ...
    def calculate_statistics(self, df):
        ...
        for project in [
            ...
            if score_col in df.columns:
                stats[f"{project}_excellent"] = len(
                    df[(df[score_col] >= 90) & (df[score_col].notna())]
                )
                ...
                stats[f"{project}_fail"] = len(
                    df[(df[score_col] < 60) & (df[score_col].notna())]
                )
            else:
        ...
        if stats["tested_total"] > 0:
            stats["excellent_ratio"] = (
                f"{stats['excellent_total']/stats['tested_total']*100:.0f}%"
            )
            stats["good_ratio"] = (
                f"{stats['good_total']/stats['tested_total']*100:.0f}%"
            )
            stats["pass_ratio"] = (
                f"{stats['pass_total']/stats['tested_total']*100:.0f}%"
            )
            stats["fail_ratio"] = (
                f"{stats['fail_total']/stats['tested_total']*100:.0f}%"
            )
    ...
    def add_student_data(self, ws, df):
        ...
        df[["班号", "格式化班名"]] = df.apply(
            lambda row: pd.Series(
                extract_class_number_and_format(row["班级名称"], row["年级"])
            ),
            axis=1,
        )
        ...
        for idx, row in df_sorted.iterrows():
            ...
            ws.cell(
                row=row_num,
                column=22,
                value=row["附加分"] if row["附加分"] > 0 else None,
            )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="体测数据转换脚本 - 生成全校学生体质健康测试成绩总表"
    )
    parser.add_argument(
        "--input-file", "-i", type=str, required=True, help="输入Excel文件路径"
    )
    parser.add_argument(
        "--output-file", "-o", type=str, required=True, help="输出Excel文件路径"
    )
...
def main():
    ...
    命令行使用示例:
    python whole_school.py --input-file "input.xlsx" --output-file "output.xlsx"
    python whole_school.py -i "input.xlsx" -o "output.xlsx"
    """
    ...
    try:
        ...
        if output_dir and not os.path.exists(output_dir):
            ...
            print(f"✓ 创建输出目录: {output_dir}".encode("utf-8"))

        # 创建转换器
        converter = NationalStandardConverter()

        print(f"✓ 开始处理文件: {args.input_file}".encode("utf-8"))
        print(f"✓ 输出文件: {args.output_file}".encode("utf-8"))

        # 转换单个Excel文件为总表
        converter.convert_single_file_to_school_format(
            args.input_file, args.output_file
        )
        print(f"✓ 成功生成全校总表: {args.output_file}".encode("utf-8"))
