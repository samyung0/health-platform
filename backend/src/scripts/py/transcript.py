...
class PersonalTranscriptGenerator:
    def __init__(self):
        self.source_file = r"25年9月体测成绩得分等级汇总/全校学生体质健康测试成绩总表.xlsx"
        ...
        try:
            ...
            if jump_score == 100:
                ...
                if actual_jumps > standard_jumps:
                    ...
                    return actual_jumps, jump_score, grade_level, extra_jumps, bonus_score
                else:
                    # 实际没超过满分，无加分
    ...
    def create_personal_transcript(self, student_data):
        ...
        # 设置列宽
        col_widths = [28, 10, 13, 10, 23, 7, 9]  # A=28, B=10, C=13, D=10, E=23, F=7, G=9
        ...
        total_score = (standard_score if pd.notna(standard_score) else 0) + (additional_score if pd.notna(additional_score) else 0)
        ...
        ws['B12'] = student_data.get('仰卧起坐值', '') if pd.notna(student_data.get('仰卧起坐值', '')) else ""
        ws['B12'].border = thin_border
        ws['C12'] = student_data.get('仰卧起坐得分', '') if pd.notna(student_data.get('仰卧起坐得分', '')) else ""
        ws['C12'].border = thin_border
        ws['D12'] = student_data.get('仰卧起坐等级', '') if pd.notna(student_data.get('仰卧起坐等级', '')) else ""
        ...
        if student_data.get('年级', '') == '六年级':
            ...
            ws['B13'] = student_data.get('50米×8往返跑值', '') if pd.notna(student_data.get('50米×8往返跑值', '')) else ""
            ws['B13'].border = thin_border
            ws['C13'] = student_data.get('50米×8往返跑得分', '') if pd.notna(student_data.get('50米×8往返跑得分', '')) else ""
            ws['C13'].border = thin_border
            ws['D13'] = student_data.get('50米×8往返跑等级', '') if pd.notna(student_data.get('50米×8往返跑等级', '')) else ""
        ...
        ws['B17'] = f"{student_data.get('标准分', ''):.1f}" if pd.notna(student_data.get('标准分', '')) else ""
        ...
        ws['E17'] = additional_score if additional_score and pd.notna(additional_score) and additional_score > 0 else ""
    ...
    def generate_all_transcripts(self):
        ...
        for idx, student in students_df.iterrows():
            try:
                ...
                # 创建年级文件夹路径
                grade_dir = os.path.join(self.output_dir, grade) if grade else self.output_dir
                
                # 创建班级文件夹路径
                class_dir = os.path.join(grade_dir, class_name) if class_name else grade_dir
