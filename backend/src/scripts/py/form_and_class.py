...
class GradeClassProcessor:
        ...
        for grade in ['四年级', '六年级']:
            for gender in ['男', '女']:
                ...
                if gender in self.lung_standards and grade in self.lung_standards[gender]:
                    ...
                    for idx in df[mask].index:
                        value = df.loc[idx, '肺活量(ml)'] if '肺活量(ml)' in df.columns else np.nan
                ...
                if gender in self.sitreach_standards and grade in self.sitreach_standards[gender]:
                    ...
                    for idx in df[mask].index:
                        value = df.loc[idx, '坐位体前屈(cm)'] if '坐位体前屈(cm)' in df.columns else np.nan
                        score = self.get_score_from_table(value, standards, reverse=False)
                ...
                if grade in ['三年级', '四年级', '五年级', '六年级']:
                    if gender in self.situp_standards and grade in self.situp_standards[gender]:
                        ...
                        for idx in df[mask].index:
                            value = df.loc[idx, '一分钟仰卧起坐(个)'] if '一分钟仰卧起坐(个)' in df.columns else np.nan
                            score = self.get_score_from_table(value, standards, reverse=False)
    ...
    def calculate_statistics(self, df):
        ...
        for project in ['BMI', '肺活量', '50米跑', '坐位体前屈', '跳绳', '仰卧起坐', '往返跑']:
            ...
            if score_col in df.columns:
                ...
                stats[f'{project}_excellent'] = len(project_scores[project_scores >= 90])
                stats[f'{project}_good'] = len(project_scores[(project_scores >= 80) & (project_scores < 90)])
                stats[f'{project}_pass'] = len(project_scores[(project_scores >= 60) & (project_scores < 80)])
    ...
    def create_class_summary(self, class_data, grade, class_num):
        ...
        # 添加统计表格
        self.add_class_statistics(ws, male_count, female_count, total_students,
                                excellent_count, good_count, pass_count, fail_count, stats, grade)
    ...
    def add_class_statistics(self, ws, male_count, female_count, total_count,
                           excellent_count, good_count, pass_count, fail_count, stats, grade):
        """添加班级统计信息"""
        ...
        # --- 合并 A8:D13 ---
        ws.merge_cells('A8:D13')  # 也可以写成 start_row=8, start_column=1, end_row=13, end_column=4
    ...
    def add_class_student_data(self, ws, class_data):
        ...
        # 学生数据表头（从第15行开始）
        headers = ['序号', '学号', '姓名', '性别', '单项指标',
                  '体重指数（BMI）（千克/米²）', '肺活量（毫升）', '50米跑（秒）',
                  '坐位体前屈（cm）', '一分钟跳绳（次）']
        ...
        # 按综合得分从高到低排序学生数据
        class_data_sorted = class_data.sort_values('综合得分', ascending=False).reset_index(drop=True)
        ...
        for idx, student in class_data_sorted.iterrows():
            ...
            ws.cell(row=start_row, column=17).alignment = Alignment(horizontal='center', vertical='center')
            
            # 合并R列（第18列）的三行
            ws.merge_cells(start_row=start_row, start_column=18, end_row=end_row, end_column=18)
            ws.cell(row=start_row, column=18, value=student.get('综合等级', ''))
            ws.cell(row=start_row, column=18).alignment = Alignment(horizontal='center', vertical='center')
    ...
    def create_grade_summary(self, grade_data, grade):
        ...
        # 添加年级统计
        self.add_grade_statistics(ws, male_count, female_count, total_students, grade_stats, grade)
        ...
        print(f"✅ 已设置数据区域格式：居中、加粗、行高25、ABCD栏宽度10、E到{get_column_letter(max_col)}栏宽度17，字体Calibri")
    ...
    def add_grade_statistics(self, ws, male_count, female_count, total_count, stats, grade):
        """添加年级统计信息"""
        # 合并第二行AB
        ...
        col_idx = 12  # L列
        if grade in ['三年级', '四年级', '五年级', '六年级']:
            cell = ws.cell(row=9, column=col_idx, value=stats.get('仰卧起坐_excellent', 0))
            cell.font = Font(bold=True, name='Calibri')
            cell.alignment = Alignment(horizontal='center', vertical='center')
            col_idx += 1
        if grade in ['五年级', '六年级']:
            cell = ws.cell(row=9, column=col_idx, value=stats.get('往返跑_excellent', 0))
            cell.font = Font(bold=True, name='Calibri')
            cell.alignment = Alignment(horizontal='center', vertical='center')
            col_idx += 1
        # 最后一列为综合等级
