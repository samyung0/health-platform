...
const withModifyableId: MiddlewareHandler<AppBindings> = async (c, next) => {
  // EXPECTS WITHSESSION MIDDLEWARE BEFORE THIS
  // all classifications should have same entity id and names
  const session = c.get("session");
  if (!session || session.activeClassifications.length === 0) {
  ...
  if (permissions.canAccessSameEntityNameInClassification) {
    ...
    const childEntity = db
      .select()
      .from(entity)
      .where(eq(entity.isChildOf, session.activeClassifications[0].entityId))
      .as("childEntity");
    const queryableClassificationIds = await db
      .select({ classificationId: classification.id })
      .from(classification)
...
export default withModifyableId;
